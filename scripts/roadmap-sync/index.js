 ...
 * $ npm run sync-roadmap -- --path=./path/to/roadmap.md
 *
 * Configuration:
 * -------------
 ...
 * 
 * You can pass a custom roadmap path using the --path argument:
 *   node scripts/roadmap-sync/index.js --path=./scripts/roadmap-sync/test-roadmap.md
 * 
 * Configuration:
 ...
 */
async function parseRoadmap(filePath, config) {
  try {
 ...
 */
async function createIssues(roadmap, config) {
  ...
  for (const epic of roadmap.epics) {
    ...
    for (const task of epic.tasks) {
      ...
      try {
        ...
        // Keep track of subtasks to update parent later
        const subtaskReferences = [];
        
        log.success(`Created issue #${taskIssue.data.number}`);
        
        // Add to project
        try {
          await addIssueToProject(taskIssue.data.node_id, config);
        ...
        for (const subtask of task.subtasks) {
          ...
          // Use GitHub's sub-issue linking syntax
          let subtaskBody = `> [!NOTE]\n> This is a sub-issue of #${taskIssue.data.number}\n\n`;
          ...
          subtaskBody += `*Generated from roadmap*\n\nSub-issue of: #${taskIssue.data.number}`;
          ...
          try {
            ...
            // Add to project
            try {
              await addIssueToProject(subtaskIssue.data.node_id, config);
              log.detail(`Added to project successfully`);
              
              // Store reference to update parent later
              subtaskReferences.push(subtaskIssue.data.number);
            } catch (projectError) {
        ...
        // After creating all subtasks, update the parent issue to list them
        if (subtaskReferences.length > 0) {
          try {
            // Get the current task issue to append to its body
            ...
            // Create a sub-issues section
            let updatedBody = currentIssue.data.body + "\n\n## Sub-issues\n";
            
            // Add task list items for each subtask
            subtaskReferences.forEach(subtaskNumber => {
              updatedBody += `- [ ] #${subtaskNumber}\n`;
            });
            ...
            log.success(`Updated parent issue #${taskIssue.data.number} with ${subtaskReferences.length} sub-issues`);
          } catch (updateError) {
            log.error(`Error updating parent issue with subtasks: ${updateError.message}`);
            hasErrors = true;
          }
        }
 ...
 */
async function addIssueToProject(issueId, config) {
  ...
  try {
    ...
    // Note: We don't need to update the title in the project separately
    // The title comes directly from the issue title, which we've already fixed
...
async function main(config) {
  try {
    ...
    const roadmap = await parseRoadmap(roadmapPath, config);
    ...
    log.info(`Roadmap Path: ${config.roadmapPath}`);
    log.info(`Repository: ${config.owner}/${config.repo}`);
    log.info(`Project ID: ${config.projectId}`);
    log.info(`Default Priority: ${config.defaults.priority}`);
    log.info(`Default Size: ${config.defaults.size}`);
    log.info(`Default Type: ${config.defaults.type}`);
    ...
    log.warning('Press (a) to accept, (c) to cancel, or wait 15 seconds to continue automatically...');
      ...
      return new Promise(resolve => {
        const onData = (data) => {
          ...
          if (byteArray.length > 0) {
            const key = String.fromCharCode(byteArray[0]).toLowerCase();
            if (key === 'a') {
              log.success('\nAccepted! Proceeding with sync...');
              cleanup();
              resolve(true);
            } else if (key === 'c') {
              log.error('\nCancelled by user.');
    ...
    // Set up a 15-second timeout that can be interrupted by keypress
    ...
    await Promise.race([keypress(), timeout]);
    
    // Create issues
    await createIssues(roadmap, config);
...
function processArgs() {
  // Look for --path=./some/path argument
...
const { customPath } = processArgs();

// Override roadmap path if specified
if (customPath) {
  CONFIG.roadmapPath = customPath;
  log.info(`Using custom roadmap path: ${customPath}`);
}
