 ...
 * Export Utilities
 * 
 * Utility functions for exporting and previewing comic panels
 */
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';
import React from 'react';
// Define ExportFormat locally to avoid JSX compilation issues
...
export const exportComic = async ({
  ...
  try {
    ...
    const tempContainer = prepareContainerForExport(containerRef.current);
    
    // Hide panel controls in the export
    const panelControls = tempContainer.querySelectorAll('.panel-controls');
    panelControls.forEach((element) => {
      (element as HTMLElement).style.display = 'none';
    });
    
    // Hide resize handles in the export
    const resizeHandles = tempContainer.querySelectorAll('.resize-handle');
    resizeHandles.forEach((element) => {
      (element as HTMLElement).style.display = 'none';
    });

    // Capture the container as a canvas
    // Get the cloned element (first child of the temp container)
    const clonedElement = tempContainer.firstChild as HTMLElement;
